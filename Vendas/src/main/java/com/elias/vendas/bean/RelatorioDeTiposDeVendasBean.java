...
public class RelatorioDeTiposDeVendasBean implements Serializable {
	...
	@PostConstruct // essa anotation diz que o metodo tem que disparar no momento em que a tela é
					// criada
	public void listar() {
	...
	public void relatorioTipoVenda() {
		ChamaReports instance = new ChamaReports();
		String tipoEscolhido = tipo;

		try {

			if (tipoEscolhido == null) {
				return;
			} else if (!tipoEscolhido.equals("Todos")) {

				instance.relatorioVenda(tipoEscolhido);
	...
	/**
	 * public void initBarModel() { ChartSeries modelVendas = new ChartSeries();
	 * 
	 ...
	 * 
	 * model.setTitle("Gráfico de vendas"); model.setLegendPosition("e");
	 * model.setStacked(true);
	 * 
	 * Axis xAxis = model.getAxis(AxisType.X); xAxis.setLabel("Valor");
	 * 
	 * 
	 * Axis yAxis = model.getAxis(AxisType.Y); yAxis.setLabel("Código");
	 * 
	 * 
	 * 
	 * }
